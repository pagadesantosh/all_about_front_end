...
async function promiseMerge(...promises) {
  if (promises.length === 0) {
    return Promise.reject(new TypeError('No promises provided'));
  }

  return Promise.all(promises).then((results) => {
    if (results.length === 0) {
      return Promise.reject(new TypeError('No values resolved from promises'));
    }

    const firstType = typeof results[0];
    let accumulator;

    // Initialize accumulator based on the first type
    if (firstType === 'number') {
      accumulator = 0;
      ...
      accumulator = [];
    } else if (firstType === 'object' && results[0] !== null) {
      accumulator = {};
    } else {
      return Promise.reject(new TypeError('Invalid or unsupported data type'));
    }

    for (let result of results) {
      if (
        typeof result !== firstType ||
        Array.isArray(result) !== Array.isArray(results[0])
      ) {
        return Promise.reject(new TypeError('Inconsistent types of promises'));
      }
      ...
      } else if (firstType === 'object' && result !== null) {
        accumulator = { ...accumulator, ...result };
      }
    }
